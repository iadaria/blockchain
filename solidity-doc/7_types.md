...
RU
Приведенный ниже пример является упрощенной версией токена (ERC20)[https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC20/ERC20.sol].
...
RU
В приведенном ниже примере предоставлены необязательные `KeyName` и `ValueName`. Это не влияет на функциональность контракта или на байткод, а только задает поле имени для входов и выходов в ABI для геттера сопоставления.
...
RU
Следующий пример использует `_allowances` для записи суммы, которую другому лицу разрешено снять с вашего счета.
...
RU
Перебирать сопоставления нельзя, т.е. нельзя перечислять их ключи. Однако возможно, реализовать структуру данных поверх них и выполнять итерацию по ней. Например, в приведенном ниже коде реализована библиотека `IterableMapping`, в которую затем добавляются данные из контракта `User`, а функция `sum` выполняет итерацию для суммирования всех значений.
...
contract User {
    ...
    // Просто стуктура для хранения наших данных
    itmap data;
    // Apply library functions to the data type.
    // Применяем библиотечные функции к типу данных
    using IterableMapping for itmap;

    // Insert something
    // Вставляем что-нибудь
    function insert(uint k, uint v) public returns (uint size) {
        // This calls IterableMapping.insert(data, k, v)
        // Вызываем `IterableMapping.insert(data, k, v)`
        data.insert(k, v);
        // We can still access members of the struct,
        // but we should take care not to mess with them.
        // Мы по-прежнему можем обращаться к членам структуры,
        // но мы должны позаботиться, чтобы не запутаться в них.
        return data.size;
    }

    // Computes the sum of all stored data.
    // Вычисляем сумму всех хранимых данных.
...
RU
Арифметические и побитовые операции могут применяться, если если оба операнда разного типа. Например, можно вычислить `y = x + z`, где `x` - `uint8`, а `z` имеет тип `uint32`. В этих случаях для определения типа, и в котором вычисляетяс операция(это важно в случае переполнения), и типа результата оператора будет использоваться следующий механизм:

1. Если тип правого операнда может быть неявно приведен к типу левого операнда, то используется тип левого операнда,
2. если тип левого операнда может быть неявно приведен к типу правого операнда, то используется тип правого операнда,
3. в противном случае, операция недопустима.
...
RU
В случае, когда один из операндов является литеральным числом, то он сначала преобразуется к своему "мобильному типу", который представляет собой самый наименьший тип, способный хранить данное значение (беззнаковый тип той же битовой ширины считаются "меньшими", чем знаковые типы). Если оба числа являются литеральными, то операция вычисляется с фактически неограниченной точность, поскольку выражение вычисляется с любой необходимой точность, чтобы не потерять ее при использовании результата с нелитеральным типом.
...
RU
Тип результата операции совпадает с типом, в котором выполняется операция, за исключением операторов сравнения, где результатом операции всегда является `bool`.

EN
The operators \*\* (exponentiation), << and >> use the type of the left operand for the operation and the result.

RU
Операторы `**`(возведение в степень), `<<` и `>>` используют для операции и результата тип левого операнда.
...
RU
Тернарный оператор используется в выражениях вида `<expression> ? <trueExpression> : </falseExpression>`. Он вычисляет одно из двух последних приведенных выражений в зависимости от результата вычисления основного `<expression>`. Если `<expression>` равно `<true>`, тогда `<trueExpression>` будет вычислено, иначе - `<falseExpression>`.
...
RU (дословно)
"Результат тернарного оператора не имеет типа рационального числа, даже если все его операнды являются литералами рациональных чисел. Тип результата определяется из типов двух операндов аналогично тому, как это было описано выше, причем при необходимости сначала выполняется преобразование к их мобильному типу."
...
RU
Как следствие, 255 + (true ? 1 : 0) вернется из-за арифметического переполнения. Причина в том, что (true ? 1 : 0) имеет тип uint8, что вынуждает выполнять сложение также в uint8, а 256 выходит за пределы допустимого для этого типа диапазона.
...
RU
Другим следствием является то, что выражение типа 1.5 + 1.5 является допустимым, а 1.5 + (true ? 1.5 : 2.5) - нет. Это связано с тем, что первое является рациональным выражением, вычисляемым с неограниченной точностью, и имеет значение только его конечное значение. Во втором случае происходит преобразование дробного рационального числа в целое, что в настоящее время запрещено.

### Compound and Increment/Decrement Operators

### Сложные операторы и операторы инкремента/декремента
...
Если `a` являетя `LValue` (т.е. переменной или чем-то, чему можно присвоить значение), то в качестве сокращений доступны следующие

EN
a += e is equivalent to a = a + e. The operators -=, \*=, /=, %=, |=, &=, ^=, <<= and >>= are defined accordingly. a++ and a-- are equivalent to a += 1 / a -= 1 but the expression itself still has the previous value of a. In contrast, --a and ++a have the same effect on a but return the value after the change.

RU
`a += e` эквивалентно `a = a + e`. `-=`, `*=`, `/=`, `%=`, `|=`, `&=`, `^=`, `<<=` и `>>=` определяются соответствующим образом. `a++` и `a--` эквивалентны `a += 1`/`a -= 1`, но само выражение имеет прежнее значение `a`. Напротив, `--a` и `++a` оказывают такой же эффект на `a`, но возращают значение уже после изменения.
...
RU
`delete a` присваивает начальное значение соответствующего типа `a`, т.е. для целых чисел это эквивалентно `a = 0`, но может также использоваться для массивов, где присваивается динамический массив нулевой дилны или статический массив той же длины, все элементы которого имеют начальное значение. `delete a[x]` удаляет элемент с индексом `x` в массиве, оставляя нетронутыми все остальные элементы и длину массива. Это, в частности означает, что в массиве остается пробел. Если вы планируете удалять элементы, то, вероятно, `mapping` будет предпочтительнее.

EN
For structs, it assigns a struct with all members reset. In other words, the value of a after delete a is the same as if a would be declared without assignment, with the following caveat:

RU
Для `struct`, назначается структура, все члены которой сброшены(обнулены). Другими словами, значение `a` после удаления `delete a` будет таким же, как если бы `a` было объявлено без присваивания, со следующей оговоркой:

EN
delete has no effect on mappings (as the keys of mappings may be arbitrary and are generally unknown). So if you delete a struct, it will reset all members that are not mappings and also recurse into the members unless they are mappings. However, individual keys and what they map to can be deleted: If a is a mapping, then delete a[x] will delete the value stored at x.

RU
`delete` не действует на `mapping` (поскольку ключи в `mapping` могут быть произвольными и в вообще неизвестны). Таким образом, при удалении структуры все ее члены, не являющиеся типом `mapping`, будут сброшены, а также произойдет перебор ее членов, за исключением типа `mapping`. Однако, отдельные ключи и то, с чем они сопоставляются, могут быть удалены: если `a` - `mapping`, тогда `delete a[x]` удалит значение, хранящееся по адресу `x`.
...
RU
Важно отметить, что `delete a` на самом деле ведет себя как присваивание `a`, т.е. сохраняет новый объект в `a`. Это различие заметно, когда `a` является ссылкой: Она сбрасывает только саму переменную `a`, а не то значение на которое она ссылалась ранее.
