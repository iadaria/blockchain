...
- функция `request` определяет условия, невыолнение которых приводит к возврату/выходу из функции; В рассматриваемом примере, `request(msg.sender == minter);` гарантирует, что данную функцию может выполнить только владелец контракта. В общем, владелец может выпускать столько токенов, сколько ему захочется, но в какой-то момент это приведет к эффекту, который называется "переполнением". Обратите внимание, что т/к по умолчанию происходит ?`проврка математических вычислений` в результате которых транзакция может быть отменена, если выражение `balances[receiver] += amount;` приведет к переполнению, т.е. когда `balances[receiver] += amount` в ?арифметических расчетах произвольной точности, становится больше чем максимально возможное значение, `uint` а именно`(2 ** 256 - 1)`. Так же работает и выражение `balances[receiver] += amount;` в функции `send`

`Errors` позволяют предоставить больше информации клиенту почему условие или операция ?g не верны. Ошибки используются в паре?/одновременно/вместе с `инструкцией revert`. Оператор `revert` безаговорочно прерывает выполенение функции и отменяет все сделанные изменения аналогично функции `require`, но он также прозволяет вам указать наименование ошибки, а также дополнитедльные данные, которые будут предоставлены вызывающей стороне(в конечном итоге, фронту приложения или навигатору блокчейн-блоков) так что можно будет легче отладить возникший сбой или отреагировать на него.
